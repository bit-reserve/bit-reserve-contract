...
contract gBTR is ERC20 {
    ...
    constructor(address _BTR, address _sBTR, address _staking) ERC20("Governance BTR", "gBTR") {
        require(_BTR != address(0), "Zero address: BTR");
        BTR = IERC20(_BTR);
        require(_sBTR != address(0), "Zero address: sBTR");
        sBTR = IERC20(_sBTR);
    ...
    function massApproval() external {
        BTR.approve(address(staking), type(uint256).max);
        sBTR.approve(address(staking), type(uint256).max);
    }
    ...
    /// @param _to      Address to send gBTR
    ...
    /// @notice         Unwrap gBTR
    /// @param _amount  Amount of gBTR to unwrap
    /// @param _to      Address to send sBTR
    function unwrap(uint256 _amount, address _to) external {
        _burn(msg.sender, _amount);
        sBTR.transfer(_to, balanceFrom(_amount));
    }

    /// @notice         Stake BTR receive gBTR
    /// @param _amount  Amount of BTR to stake
    /// @param _to      Address to mint gBTR
    ...
    /// @notice         Untake gBTR receive BTR
    /// @param _amount  Amount of gBTR to unstake
    /// @param _to      Address to send BTR
    ...
    function getPriorVotes(
        ...
        require(
            ...
            "gBTR::getPriorVotes: not yet determined"
        );
